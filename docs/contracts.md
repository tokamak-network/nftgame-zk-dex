...
Base contract providing UTXO-style note management and nullifier tracking, shared by F1 and F5.
...
Both interfaces have the same signature (5 public inputs):
...
> Both have the contract name `Groth16Verifier` (snarkjs default). Use fully qualified names in Hardhat:
> ```javascript
...
3. Deploy `Groth16Verifier` (from `GamingItemTradeVerifier.sol`)
4. Deploy `GamingItemTrade(verifierAddress)`
